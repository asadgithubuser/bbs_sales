...
class AdminSearchContoller extends Controller
{
    ...
    public function searchAjaxApprovedList(Request $request){
        $calender = CourseCalendar::where(['status' => 1, 'is_approved' => 1, 'fiscal_year_id' => $request->fiscal_id])->latest()->paginate(25);
        $depertments = Department::where('status', 1)->get();
        $page = View('backend.admin.calendar.ajax.tableBody',['calender' => $calender, 'type' => 'approved', 'depertments' => $depertments])->render();
    ...
    public function searchAjaxTableList(Request $request, $type){
        $q = $request->q;
        if($type == "allCourse")
        {
            $courses = TrainingCourse::where('forward', 0)->where('is_published', 0)->where('fiscal_year_id',$q)->with('trainer', 'courseDuration', 'courseCurriculam')->latest()->paginate(25);
        }else{
            $courses = TrainingCourse::where('forward', 1)->where('is_published', 0)->where('fiscal_year_id',$q)->with('trainer', 'courseDuration', 'courseCurriculam')->latest()->paginate(25);
            
        }

        $fiscal = FiscalYear::find($q);


        $page = View('backend.admin.trainingCourse.ajax.tableBody',['courses' =>$courses, 'fiscal' => $fiscal, 'q'=>$q, 'type' => $type])->render();
