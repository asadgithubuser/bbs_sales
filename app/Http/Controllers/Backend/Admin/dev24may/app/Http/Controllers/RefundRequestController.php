...
class RefundRequestController extends Controller
{
    ...
    public function request_store(Request $request, $id)
    {
        ...
        $refund->bank_credentials = json_encode($request->bank_credentials);
        $refund->mfs_credentials = json_encode($request->mfs_credentials);

        if($request->reason_select==''){
        ...
        if($refund->refund_method=='Bank'){
             foreach($request->bank_credentials as $t1){
                 if($t1==''){
                    flash( translate("Please fill in all required fields of Bank Credentials") )->error();
                    return back();
                 }                
             }
        }elseif($refund->refund_method=='Mobile Financial Service'){
            foreach($request->mfs_credentials as $t1){
                if($t1==''){
                   flash( translate("Please fill in all required fields of Mobile Financial Service") )->error();
                   return back();
                }                
            }
        }
