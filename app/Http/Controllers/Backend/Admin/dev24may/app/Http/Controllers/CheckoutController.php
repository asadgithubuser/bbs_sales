...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        //special shipping
        $special_category_flag_initialize=Session::put('special_category_flag','initialize');

		$requestj=session::forget('already_used');  //added feb 19
...
{
	$coupon = Coupon::where('code', $request->code)->where('role',0)->first();
		...
		if (strtotime(date('d-m-Y')) >= $coupon->start_date && strtotime(date('d-m-Y')) <= $coupon->end_date) {
			if (CouponUsage::where('user_id', Auth::user()->id)->where('coupon_id', $coupon->id)->first() == null) {
				...
				if ($coupon->type == 'cart_base'){
					...
					$shipping = 0;
					$carts = Cart::where('user_id', Auth::user()->id)->get();
					$pID = Cart::where('user_id', Auth::user()->id)->pluck('product_id');
					foreach ($carts as $key => $cartItem) {
						$subtotal += $cartItem['price'] * $cartItem['quantity'];
						$tax += $cartItem['tax'] * $cartItem['quantity'];
						//$shipping += $cartItem['shipping_cost'];
					...
					foreach($coupon_details as $detail){
						array_push($coupon_pId,$detail->product_id);
					}

					$carts = Cart::where('user_id', Auth::user()->id)
						->whereIn('product_id',$coupon_pId )
						->get();
