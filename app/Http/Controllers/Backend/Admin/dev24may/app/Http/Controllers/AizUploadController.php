...
class AizUploadController extends Controller
{
    ...
    {    $temp_array= explode('/',$request->_current_img_id);
         $data = DB::table('uploads')->where('id', end($temp_array))->first();

         $file_name=$data->file_name;
         $extension=$data->extension;
         $new_name = str_replace($extension, "jpeg", $file_name);
 

         //DB::insert('insert into 1test (value) values (?)', [$extension]);
        
         $image = $request->img_data;  // your base64 encoded
         ...
         $temp_name=str_replace('uploads/all/', '', $new_name); $temp_name=str_replace('/', '', $temp_name);
         ...
         if($request->operation=='replace'){
            ...
            ->updateOrInsert(
                ['id' => end($temp_array)],
                ['file_name' => $new_name,'extension' => 'jpeg', 'file_size'=>$size]
            );             
         }else{
            $new_name='/uploads/all/'.$rand.$temp_name;
            
            DB::insert('insert into uploads (file_original_name,file_name,user_id,file_size,extension,type) values (?, ?, ?, ?, ?, ?)', [$data->file_original_name,$new_name,Auth::user()->id,$size,'jpeg','image']);
         }

        \File::put(public_path().'/uploads/all/'.$rand.$temp_name, base64_decode($image));
        
        //DB::insert('insert into 1test (value) values (?)', [$temp_name]);
