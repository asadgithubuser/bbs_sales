...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('frontend.index', compact('flash_deal', 'featured_categories'));
    }
    ...
    public function dashboard()
    {
        ...
        //
        $this_user_review = DB::table('reviews')
		...
		foreach($this_user_review as $this_user_review_each){
			$pro_col[$i++]=$this_user_review_each->product_id; 
		}	
							
	

        $product_delivered = DB::table('order_details')
                    ->orderBy('order_details.id', 'desc')
                    ->join('products', 'order_details.product_id', '=', 'products.id')
                    ->join('orders', 'orders.id', '=', 'order_details.order_id')
                    ->where('orders.user_id', Auth::user()->id)
					->whereNotIn('products.id', $pro_col)
                    ->where('order_details.delivery_status', 'delivered')
                    ->select('products.id as pid','products.name as p_name','products.refundable as refundable','products.slug as slug','order_details.quantity as qty','order_details.shipping_type as ship','order_details.price as price')
                    ->paginate(3);
        ...
        if(Auth::user()->user_type == 'seller'){
            ...
            ->selectRaw('count(products.id) as pro_count, categories.name as category_name')
            ->where('products.user_id', '=', Auth::user()->id)
            ->groupBy('products.category_id')
            ->get();

            return view('frontend.user.seller.dashboard', compact('productCount','product_delivered')); 
        }

        elseif(Auth::user()->user_type == 'customer'){
            return view('frontend.user.customer.dashboard', compact('product_delivered'));
        }
        elseif(Auth::user()->user_type == 'delivery_boy'){
            return view('delivery_boys.frontend.dashboard', compact('product_delivered'));
        }
        else {
            abort(404);
        }
    ...
    {
        $reviews = Review::orderBy('created_at', 'desc')->where('user_id', Auth::user()->id)->paginate(5);
        //return view('backend.product.reviews.index', compact('reviews'));
        return view('frontend.user.product_review', compact('reviews'));
    }
    ...
    public function userProfileUpdate(Request $request)
    {
        ...
        if($request->name == '' || $request->name == null){
            flash(translate('Please enter your name!'))->error();
            return back();
        }else{
        ...
        /*
        if( is_numeric($request->phone) && strlen(strval($request->phone))==11 ){
            $user->phone = '+88'.$request->phone;
        }else if(is_numeric($request->phone) && strlen(strval($request->phone))==14){
            $user->phone = $request->phone;
        }else if(is_numeric($request->phone) && strlen(strval($request->phone))<=10){
        ...
        if($request->confirm_password!='')
        {
        	if($request->new_password != null && ($request->new_password == $request->confirm_password)){
        		...
        		flash(translate('Password not match'))->error();
        		return back();
        	}	
        }
    ...
    {
        $flash_deal = FlashDeal::where('slug', $slug)->first();
        $flash_deal_products = FlashDealProduct::where('flash_deal_id','=',$flash_deal->id)->inRandomOrder()->paginate(30);
        if($flash_deal_products != null)
            return view('frontend.flash_deal_details', ['flash_deal'=>$flash_deal,'flash_deal_products'=>$flash_deal_products]);
        else {
            abort(404);
        }
    }
	...
	/*Added Home page Design*/
    public function load_new_products_section(){
        return view('frontend.partials.new_products_section');
    }

    public function load_todays_deal_section(){
        return view('frontend.partials.todays_deal_products');
    }
	/*End Added Home page Design*/
...
//        $categories = Category::where('level', 0)->orderBy('name', 'asc')->get();
    ...
    public function pacTrack(Request $request){
        $pac = new PacTracking();
        $pac->domain_name = $request->domain_name;
        $pac->url = $request->url;
        $pac->user_id = (Auth::check())? Auth::user()->id: 0;
        $pac->save();
        } 
}
