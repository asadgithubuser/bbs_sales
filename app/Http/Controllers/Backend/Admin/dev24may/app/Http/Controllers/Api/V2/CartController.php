...
//Error: product minimum qty smaller than request qty
        if ($product->min_qty > $request->quantity) {
            return response()->json(['result' => false, 'message' => translate("Minimum")." {$product->min_qty} ".translate("item(s) could be ordered")], 200);
        }
		
 		
//Error: product max qty not 0 and product max qty smaller than request qty
		if(($product->max_qty!=0) && ($product->max_qty < $request->quantity)){
            return response()->json(['result' => false, 'message' => translate("Maximum")." {$product->max_qty} ".translate("item(s) could be ordered")], 200);
        }  
		...
		$cart_update=true;
		$get_already_added_qty = DB::table('carts')->where('user_id','=',auth()->user()->id)->where('owner_id','=',$product->user_id)->where('product_id','=',$request->id)->where('variation','=',$variant)->first()->quantity ?? 0;
		
		if(($product->max_qty!=0) && (($get_already_added_qty+$request->quantity) > $product->max_qty))
		{
			$cart_update=false;
			return response()->json(['result' => false, 'message' => translate("You already have this iteam in the cart. Maximum")." {$product->max_qty} ".translate("item(s) could be ordered")], 200);
		}
    ...
    public function changeQuantity(Request $request)
    {
        ...
        if ($cart != null) {
			...
			if(($cart->product->max_qty!=0) &&  ($request->quantity > $cart->product->max_qty) ){
				return response()->json(['result' => false, 'message' => translate("Maximum")." {$cart->product->max_qty} ".translate("item(s) could be ordered for")." {$cart->product->name}"], 200);
			}
...
if(($product->max_qty!=0) && ($product->max_qty < $cart_quantities[$i])){
		 return response()->json(['result' => false, 'message' => translate("Maximum")." {$product->max_qty} ".translate("item(s) could be ordered for")." {$product->name}"], 200);

}else{
	    if ($product->min_qty > $cart_quantities[$i]) {
                    return response()->json(['result' => false, 'message' => translate("Minimum")." {$product->min_qty} ".translate("item(s) could be ordered for")." {$product->name}"], 200);
                }
