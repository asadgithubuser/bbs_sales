...
@if($product->update_history)						
<p>
  <a class="" href="#" type="button" data-toggle="collapse" data-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
    Previous Price ({{count(json_decode($product->update_history))}})
  </a>
</p>
@endif
					...
					@foreach(json_decode($product->update_history) as $each_act)
						<tr>
							@foreach(explode(',',$each_act) as $each_ind)
							@php $arr_each_ind=explode('=',$each_ind); @endphp
							   <td>
								 @php
								 
									 if($arr_each_ind['0']=='user'){ echo user_by_id($arr_each_ind['1'])->name; } else { 		  
										if(empty($arr_each_ind['1'])){ echo $arr_each_ind['0']; }else{ echo $arr_each_ind['1']; }
									 }  
									 
									 if($arr_each_ind['0']=='unit_price'){
										$this_price=$arr_each_ind['1'].'.00';
									 }
									 if($arr_each_ind['0']=='updated_at'){
										$updated_at=$arr_each_ind['1'];
									 }
								 @endphp
							   </td>
							...
							@endforeach
							   <td>
							   @php
							      echo $count_order = DB::table('order_details')->where('product_id','=',$product->id)
								  ->where('price','=',$this_price)
