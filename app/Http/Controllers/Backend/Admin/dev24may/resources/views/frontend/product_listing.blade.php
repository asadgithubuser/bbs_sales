            ...
            <form class="" id="search-form" action="" method="GET">
                <div class="row">
                    ...
                    <div class="col-xl-10">
                            ...
                            <div class="row gutters-5 flex-wrap align-items-center">
                                ...
                                <div class="col-6 col-lg-auto mb-3 w-lg-200px">
                                    ...
                                    <select class="form-control form-control-sm aiz-selectpicker" name="sort_by" onchange="filter()">
                                        @if($query) <option value="best_matching" @isset($sort_by) @if ($sort_by == 'best_matching') selected @endif @endisset>{{ translate('Best Matching')}}</option>@endif
						...
						{{--						
 @php
	$parent = App\Models\Category::where('id',$category_id)->pluck('id');
	$child = App\Models\Category::whereIn('parent_id',$parent)->pluck('id');
	$sub_child = App\Models\Category::whereIn('parent_id',$child)->pluck('id');
	$sub_sub_child = App\Models\Category::whereIn('parent_id',$sub_child)->pluck('id');

	$ids = array();
	array_push($ids,$parent[0]);

	foreach ($child as $id){
		array_push($ids,$id);
	}
	foreach ($sub_child as $id){
		array_push($ids,$id);
	}
	foreach ($sub_sub_child as $id){
		array_push($ids,$id);
	}
	$banners = DB::table('categories')
				->leftJoin('uploads', 'categories.banner', '=', 'uploads.id')
				->whereIn('categories.id',$ids)->get();
@endphp
<div class="row gutters-5 row-cols-xxl-6 row-cols-xl-3 row-cols-lg-6 row-cols-md-3 row-cols-2 justify-content-md-center mb-3">
	@foreach( $banners as $banner)
		@if($banner->banner != 'NULL' && $banner->banner != null)
			<div class="col">
				<div class="aiz-card-box border border-light rounded hov-shadow-md mt-1 mb-2 has-transition bg-white">
					<div class="position-relative">
						<a href="{{route('products.category',$banner->slug)}}" class="d-block">
							<img class="img-fit lazyload mx-auto h-140px h-md-210px" src="{{ static_asset($banner->file_name) }}"/>
						</a>
					</div>
				</div>
			</div>
	...
	@endforeach
                        ...
                        <div class="row gutters-5 row-cols-xxl-6 row-cols-xl-3 row-cols-lg-6 row-cols-md-3 row-cols-2">
                        ...
                        <!-- Showing SEO article here -->
						{{-- <div class="seo-article-section d-none d-lg-block mt-4 pb-2">
                            <div class="seo-article-area pt-2 pb-4">
                                <div class="seo-article">
                                    {!! $category->seo_article !!}
                                </div>
                            </div>
                        </div>--}}
                     <!-- END Showing SEO article here -->
