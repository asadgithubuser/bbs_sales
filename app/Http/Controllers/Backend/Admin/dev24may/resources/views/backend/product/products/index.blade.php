            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="lg">{{translate('Top Product')}}</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_new_product(this)" value="{{ $product->id }}" type="checkbox" <?php if ($product->new == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_top_product(this)" value="{{ $product->id }}" type="checkbox" <?php if ($product->top_product == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        <td>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_new_product(el){
            if(el.checked){
                var status = 1;
            }
            else{
            ...
            $.post('{{ route('products.update_new_product') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('New products updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
        ...
        function update_top_product(el){
            if(el.checked){
                var status = 1;
            }
            else{
            ...
            $.post('{{ route('products.update_top_product') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Top products updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
