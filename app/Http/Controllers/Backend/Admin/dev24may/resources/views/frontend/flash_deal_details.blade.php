            ...
            <div class="container">
                ...
                <div class="row gutters-5 row-cols-xxl-6 row-cols-xl-5 row-cols-lg-4 row-cols-md-3 row-cols-2">
                    @foreach ($flash_deal_products as $flash_deal_product)
                        @php $product = \App\Models\Product::find($flash_deal_product->product_id); @endphp
                        @if($product != null)
                            <div class="col mb-2">
                ...
                <div class="aiz-pagination aiz-pagination-center mt-4">{{ $flash_deal_products->links() }}</div>
            </div>
        </section>
    </div>
    @else
