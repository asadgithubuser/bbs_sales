    ...
    <div class="container">
        <div class="row">
                ...
                <div>
                    <a href="{{ route('home') }}" class="d-block">
                        @if(get_setting('footer_logo') != null)
                            <img class="lazyload" src="{{ static_asset('assets/img/placeholder-home-banner.png') }}" data-src="{{ uploaded_asset(get_setting('footer_logo')) }}" alt="{{ env('APP_NAME') }}" height="70">
                        @endif
                    </a>
                    ...
                    <div class="mt-4 mb-4">
                        <h4 class="fs-13 text-uppercase fw-600 border-bottom border-gray-900 pb-2">
                                {{ translate('Social') }}
                        </h4>
            ...
            <div class="col-md-3 col-lg-3">
                <div class="text-center text-md-left mt-4">
                    ...
                    <ul class="list-unstyled">
                        @if (Auth::check())
                            <li class="mb-2">
                                <a class="opacity-70 hov-opacity-100 text-reset hov-opacity-100 text-reset" href="{{ route('logout') }}">
                                    {{ translate('Logout') }}
                                </a>
                            </li>
                        @else
                            <li class="mb-2">
                                <a class="opacity-70 hov-opacity-100 text-reset hov-opacity-100 text-reset" href="{{ route('user.login') }}">
                                    {{ translate('Login') }}
                                </a>
                            </li>
                        @endif
                        <li class="mb-2">
                            <a class="opacity-70 hov-opacity-100 text-reset hov-opacity-100 text-reset" href="{{ route('purchase_history.index') }}">
                                {{ translate('Order History') }}
                            </a>
                        </li>
                        <li class="mb-2">
                            <a class="opacity-70 hov-opacity-100 text-reset hov-opacity-100 text-reset" href="{{ route('wishlists.index') }}">
                                {{ translate('My Wishlist') }}
                            </a>
                        </li>
                        <li class="mb-2">
                            <a class="opacity-70 hov-opacity-100 text-reset hov-opacity-100 text-reset" href="{{ route('orders.track') }}">
                                {{ translate('Track Order') }}
                            </a>
                        </li>
                        @if (addon_is_activated('affiliate_system'))
                            <li class="mb-2">
                                <a class="opacity-70 hov-opacity-100 text-reset hov-opacity-100 text-light" href="{{ route('affiliate.apply') }}">{{ translate('Be an affiliate partner')}}</a>
                            </li>
                        @endif
                    </ul>
                </div>
                            ...
                            <ul class="list-unstyled">
                                @if ( get_setting('widget_one_labels',null,App::getLocale()) !=  null )
                                    @foreach (json_decode( get_setting('widget_one_labels',null,App::getLocale()), true) as $key => $value)
                                        <li class="mb-2">
                                            <a href="{{ json_decode( get_setting('widget_one_links'), true)[$key] }}" class="opacity-70 hov-opacity-100 text-reset  text-reset">
                                                {{ $value }}
                                            </a>
                                        </li>
                                    @endforeach
            ...
            <div class="col-lg-3 ml-xl-auto col-md-3 mr-0">
                <div class="text-center text-md-left mt-4">
                    <h4 class="fs-13 text-uppercase text-white fw-600 border-bottom border-gray-900 pb-2 mb-4">
                        {{ translate('Contact Us') }}
                    </h4>
                    <ul class="list-unstyled">
                        <li class="mb-2">
                           <span class="d-block opacity-70">{{ translate('Address') }}:</span>
                           <span class="d-block opacity-70 hov-opacity-100 text-reset">{{ get_setting('contact_address',null,App::getLocale()) }}</span>
                        </li>
                        <li class="mb-2">
                           <span class="d-block opacity-70">{{translate('Phone')}}:</span>
                           <span class="d-block opacity-70 hov-opacity-100 text-reset">{{ get_setting('contact_phone') }}</span>
                        </li>
                        <li class="mb-2">
                           <span class="d-block opacity-70">{{translate('Email')}}:</span>
                           <span class="d-block opacity-70 hov-opacity-100 text-reset">
                ...
                <div class="text-center text-md-right">
                    <ul class="list-inline mb-0">
                        @if ( get_setting('payment_method_images') !=  null )
                            <li class="list-inline-item">{{ translate('Pay With')}} </li>
...
@if(get_setting('about_us_description') !='')
<!-- FOOTER Bottom Text Starts-->
<div class="container-fluid mt-4 d-none d-lg-block">
    <div class="row align-items-center">
        <div class="col-12">{!! html_entity_decode(get_setting('about_us_description', null, App::getLocale())) !!}</div>
    </div>
</div>
        ...
        <div class="col-auto">
            <a href="{{ route('cart') }}" class="text-reset d-block text-center pb-2 pt-3">
                <span class="align-items-center bg-primary border border-white border-width-4 d-flex justify-content-center position-relative rounded-circle size-50px" style="margin-top: -33px;box-shadow: 0px -5px 10px rgb(0 0 0 / 15%);border-color: #fff !important;">
                    <i class="las la-shopping-bag la-2x opacity-70 hov-opacity-100 text-reset"></i>
                </span>
