...
@section('content')
    <section class="mb-4 pt-3">
        <div class="container">
            <div class="bg-white shadow-sm rounded p-3  @php  $title=$time=''; $camp=is_flashdeal_product($detailedProduct->id); if($camp !=false){ $title=$camp->title; $time=date('Y/m/d H:i:s', $camp->end_date); }  @endphp ">
                    ...
                    @if($title!='')
                        <div class="pricebox">
                            <div class="campaign_tag">
                                <span class="pricebox__actual">{{$title}}</span>
                                <span class="pricebox__savings"><small class="iconx">Ending in</small><div class="aiz-count-down ml-auto ml-lg-3 align-items-center pulse" data-date="{{$time}}" ></div></span>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="col-md-12 p-0">
                        <div class="bg-white shadow-sm mb-3">
                            <div class="position-relative   text-left">
                                 <div class="  border-bottom fs-16 fw-600">{{ translate('Delivery')}}</div>
                                 

                                 <div class="pt-2">
                                    <div><i class="fa fa-truck"></i> <span class="fw-600">Home Delivery</span></div>
                                    <div><i class="las la-history aiz-side-nav-icon"></i> In side Dhaka Metro: 1 to 5 days</div>
                                    <div><i class="las la-history aiz-side-nav-icon"></i> Out side Dhaka: 3 to 7 days</div>
                                    <div class="pt-2"><i class="las la-money-bill-wave-alt aiz-side-nav-icon"></i> <span class="fw-600">Cash on Delivery Available</span></div>
                                </div>
            ...
            @if($detailedProduct->refundable)
                @php $refund_file=\APP\Models\BusinessSetting::join('uploads', 'business_settings.value', '=', 'uploads.id')->where('business_settings.type','refund_sticker')->get(['uploads.file_name'])->first()->file_name; @endphp
                <div class="col-md-12 p-0">
                    <div class="bg-white shadow-sm mb-3">
                        <div class="position-relative   text-left">
                             <div class="  border-bottom fs-16 fw-600">{{ translate('Service')}}</div>                               
                             <div class="pt-2">      
                                <i class="las la-backward aiz-side-nav-icon"></i> {{\APP\Models\BusinessSetting::where('type','refund_request_time')->get(['value'])->first()->value}} Days Returns                          
                                 {{--<img class="img-fit      ls-is-cached lazyloaded" src="{{static_asset($refund_file)}}">--}}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-12 p-0 pt-3">
                    ...
                    <div class="bg-white shadow-sm mb-3">
                        <div class="position-relative   text-left">
                            ...
                            @endif
                            <div class="  border-bottom fs-16 fw-600">{{ translate('Sold by')}}</div>
                        ...
                        <div class="row no-gutters align-items-center border-top">
                            <div class="col">
                                <a href="{{ route('shop.visit', $detailedProduct->user->shop->slug) }}" class="d-block btn-xs text-center btn-soft-primary rounded-0">{{ translate('Visit Store')}}</a>
                            </div>
                            {{--<div class="col">
    ...
    <section class="mb-4">
        <div class="container">
            <div class="row gutters-10">
				...
				@if(empty($detailedProduct->user->shop->name))
				@else
                    <div class="bg-white rounded shadow-sm mb-3">
                        <div class="p-3 border-bottom fs-16 fw-600">
                            <a href="{{ route('shop.visit', $detailedProduct->user->shop->slug) }}" class="text-reset d-block fw-600"><span>
                                {{ $detailedProduct->user->shop->name }}
                                @if ($detailedProduct->user->seller->verification_status == 1)
                                    <span class="ml-2"><i class="fa fa-check-circle" style="color:green"></i></span>
                                @else
                                    <span class="ml-2"><i class="fa fa-times-circle" style="color:red"></i></span>
                                @endif
                             /</span><span> {{ translate('Top Selling Products')}} </span></a>
                ...
                <div class="col-xl-9 order-0 order-xl-1">
                        ...
                        <div class="tab-content pt-0">
                            <div class="tab-pane fade active show" id="tab_default_1">
                                <div class="p-4 text-container">
                                    <div id="desc_container" class="mw-100 overflow-hidden content text-left aiz-editor-data">
                                        <?php echo $detailedProduct->getTranslation('description'); ?> 
                                    </div>
									
                                </div>
								<div class="show-more" id="showmore" style="display:none">
									<a href="javascript:void(0);" class="pointer h5">Show more</a>
								</div>
                            </div>
							...
							<div class="tab-pane fade" id="tab_default_3">
								<div>
									<div style="left: 0px; width: 100%; height: 0px; position: relative; padding-bottom: 141.4227%;">
										<iframe src="{{ uploaded_asset($detailedProduct->pdf) }}" frameborder="0" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true" style="top: 0px; left: 0px; width: 100%; height:20rem; position: absolute;"></iframe>
									</div>
								</div>
							</div>
					...
					{{--Start Review--}}
		                        ...
		                        <div class="nav border-bottom aiz-nav-tabs nav border-bottom aiz-nav-tabs">
									<a href="#tab_default_3"  data-toggle="tab" class="p-3 fs-16 fw-600 text-reset active">{{ translate('Product Reviews')}}</a>
                                </div>			

                                 @php $review_counter=0; @endphp

                                    <ul class="list-group list-group-flush"  id="review{{user()}}">
                                        @foreach ($detailedProduct->reviews as $key => $review)  

                                        @php 
                                            $this_review_show=1;
                                            if($review->status ==0){$this_review_show=0; }
											
											
											
											if(empty(Auth::user()->user_type)){}else{
                                            if(Auth::user()->user_type!='customer' || Auth::user()->id==$review->user_id){$this_review_show=1; }
                                            if($review->status ==0 && Auth::user()->user_type=='seller'){$this_review_show=0; }
											}
                                        ...
                                        @endphp
                                            ...
                                            @if($this_review_show==1)
                                            @php $review_counter++; @endphp



                                            
											<li class="media list-group-item d-flex " @if(($review->status ==0) && ($review->user_id!=user()))  @endif  >
                                                ...
                                                <div class="media-body text-left">
                                                    <div class="d-flex justify-content-between">
														...
														@if(($review->status ==0) && ($review->user_id==user()))
														<span class="badge badge-inline badge-danger ">Pending for Approval</span>
														@endif    
														
														
                                                        @if($review->user_id==user())
                                                        <span>
                                                            <button class="btn btn-sm btn-soft-primary" onclick="edit_review({{user()}})"><i class="las la-edit"></i> Edit</button>
                                                        </span>
                                                        @endif    
                                    ...
                                    @if($review_counter== 0)
                                        <div class="text-center fs-18 opacity-70 p-3">
                                    ...
                                    @if(Auth::check())
                                        @php
                                            ...
                                            Session::put('review_edit',1);
                                            
                                        @endphp
                                        @foreach ($detailedProduct->orderDetails as $key => $orderDetail)
                                            @if($orderDetail->order != null && $orderDetail->order->user_id == user() && $orderDetail->delivery_status == 'delivered' && 
                                            (\App\Models\Review::where('user_id', user())->where('product_id', $detailedProduct->id)->first() == null ||
                                            Session::get('review_edit')==1))
                                                @php
                                                    $commentable = true;
                                                    
                                                    $review=\App\Models\Review::where('user_id', user())->where('product_id', $detailedProduct->id)->first(); 
                                                    $review_rating=''; $review_comment=''; $review_photos='';
                                                    if($review!=''){$review_rating=$review->rating; $review_comment= $review->comment; $review_photos=$review->photos;}
                                                     
                                                @endphp
                                            @endif
                                        @endforeach
                                        @if ($commentable)
                                            <div class="p-4" @if($review_comment!='') style="display:none"   @endif  id="review_form">
                                                ...
                                                <form class="form-default" role="form" action="{{ route('reviews.store') }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="order_code" value="{{ $orderDetail->order->id }}">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div class="rating rating-input">
                                                            <label>
                                                                <input type="radio" name="rating" value="1" required @if($review_rating==1) checked  @endif>
                                                                <i class="las la-star"></i>
                                                            </label>
                                                            <label>
                                                                <input type="radio" name="rating"  value="2"  @if($review_rating==2) checked  @endif>
                                                                <i class="las la-star"></i>
                                                            </label>
                                                            <label>
                                                                <input type="radio" name="rating"  value="3"  @if($review_rating==3) checked  @endif>
                                                                <i class="las la-star"></i>
                                                            </label>
                                                            <label>
                                                                <input type="radio" name="rating" value="4"  @if($review_rating==4) checked  @endif>
                                                                <i class="las la-star"></i>
                                                            </label>
                                                            <label>
                                                                <input type="radio" name="rating" value="5"  @if($review_rating==5) checked  @endif>
                                                                <i class="las la-star"></i>
                                                            </label>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <textarea class="form-control" rows="4" name="comment" placeholder="{{ translate('Your review')}}" required>{{$review_comment}}</textarea>
                                                    </div>
...
{{-- Auth::user()->avatar_original --}}
                                                    <div class="form-group row">
                                                        <label class="col-md-3 col-form-label" for="signinSrEmail">{{translate('Photo')}} <small>(Multiple/Single)</small></label>
                                                        <div class="col-md-12">
                                                            <div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="true">
                                                                ...
                                                                <input type="hidden" name="photos" value="{{$review_photos}}" class="selected-files">
                                                            </div>
                                                            <div class="file-preview box sm">
                                                            </div>
                                                            <small class="text-muted">{{translate('These images are visible in product details review section .')}}</small>
                                                        </div>
                                                    </div>
                                                    <div class="text-right">
                                                        <span>
                                                            <x  class="btn   btn-soft-danger mt-3 mr-2" onclick="edit_review_cancel({{user()}})"><i class="las la-edit"></i> Cancel</x>
                                                        </span>		
                                                        <button type="submit" class="btn btn-primary mt-3">
                                                            @if($review_comment!=''){{ translate('Update review')}} @else {{ translate('Submit review')}} @endif
                                                        </button>
                                                    </div>
                                                </form>
                                            </div>
...
@section('script')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var divheight = $('#desc_container').innerHeight();
            if(divheight>250){
                $('#showmore').css("display","block");
                $('#desc_container').addClass("hideContent");
            }
            $(".show-more a").on("click", function() {
				...
				$("#desc_container").toggleClass("hideContent", "showContent"); 
                $this.text(linkText);
            });
            ...
            // Start comment see more
            $(".comment").shorten({
                "showChars" : 250,
                "moreText"	: "See More",
                "lessText"	: "Less",
            });
            // End comment see more
    	});
        ...
        /*Start review image*/
        var pre='';
        function image_view(data,id){
            $("#main"+pre).removeClass("photo_box"); 
            pre=data;
            var dd=$("#"+data).html();
            let res = dd.replace(/100px/g, "400px");
             res = res.replace(/onclick/g, "");
            
            $("#place"+id).fadeToggle( "fast", function() {
                $("#place"+id).html(res).fadeToggle();
            });

            
            $("#main"+data).addClass("photo_box"); 
        }

        function edit_review(user){ 
            $("#review"+user).fadeOut( "fast");
            $("#review_form").fadeIn( "fast");
        }
		
        function edit_review_cancel(user){ 
            $("#review_form").fadeOut( "fast");
            $("#review"+user).fadeIn( "fast");
        }
       /*End review image*/
