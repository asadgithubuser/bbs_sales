...
{{-- Start edit linked product --}}
<div class="form-group row">
	<label class="col-md-3 col-from-label">{{translate('is
   Linked')}}</label>
	<div class="col-md-1">
	<label class="aiz-switch aiz-switch-success mb-0">
	<input value="1" type="checkbox" name="isLinked"
   @if($product->isLinked === 1) checked @endif>
	<span></span>
	</label>
	</div>
	<div class="col-md-7">
	<input type="text" class="form-control"
   name="product_link" id="product_link" placeholder="{{translate('Product Link')}}"
   value="{{ $product->product_link }}" style="{{ ($product->isLinked === 1)? '':
   'display:none' }}"
	@if($product->isLinked === 1) required @endif>
	</div>
	</div>
	{{-- End edit linked product --}}
...
if(e.target.getAttribute('name')=="isLinked")
