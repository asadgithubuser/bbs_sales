...
<div class="row gutters-10 m-1 mb-3" id="this_review" style="background-color: #ffffff; display:none" >
            <div class="card-body">
				...
				<thead>
					<tr>
						...
						<th data-breakpoints="lg">{{translate('Review')}}</th>
					</tr>
				</thead>
				<tbody>
	
 
		@foreach($product_delivered as $key => $product_del)
			    <script>document.getElementById("this_review").style.display = "block";</script>
					<tr><td>{{ $key+1 }}</td>
					<td>{{ $product_del->p_name }}</td>
					<td>{{ $product_del->qty }}</td>
					<td>{{translate('Deliverd')}}</td>
					<td>{{ $product_del->price }}</td>

					<td>
						<a href="{{ route('product', $product_del->slug) }}"  target="_blank" class="px-2 text-primary v-btn v-btn--text theme--light v-size--small">
						<span class="v-btn__content"> Write a review</span></a>
					</td>
			 </tr>
		@endforeach
				</tbody>
				...
				{{ $product_delivered->appends(request()->input())->links() }}	 
				</div>
            </div>
            </div>
