...
class DynamicDependentController extends Controller
{
    ...
    {
        $crops = Crop::where('type', $request->useLandType)->get();
    ...
    {
        // dd($request->land_type);
        $data = $request->all();
        $cropCode = Crop::where('id', $data['crop_id'])->select('code')->first();
    ...
    public function getItemsByService(Request $request)
    {
        ...
        foreach($service_id as $item){
            ...
            $serviceItemList .=  '<div class="s_item_list'.$item.' mb-3"><select class="form-control select-multiple service_item_select" onChange="itemSelect(this)" name="service_item_selected[]" multiple>';
    ...
    public function getItemsByServiceItem(Request $request)
    {
        ...
        foreach($service_id as $item){
            ...
            $serviceItemList .=  '<div class="s_item_list'.$item.' mb-3"><select class="form-control select-multiple service_item_select" onChange="inventoryItemSelect(this)" name="service_inventory_item_id[]" multiple required>';
    ...
    public function getItemPriceByService(Request $request)
    {
        ...
        if ($data['usage_type'] == '1') {
            ...
            $serviceItemList .= '<span class="text-right" style="display: block;margin-bottom: 12px;"> Select User Type</span>';
        }
            ...
            if ($serviceItem->service_id == 1) {
                if ($data['usage_type'] == 1) {
                    if ($serviceItem->service_item_type == 1) {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItemPrice[1]->price.'">'. number_format($serviceItemPrice[1]->price, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    } else if ($serviceItem->service_item_type == 2) {
                            $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItemPrice[0]->price.'">'. number_format($serviceItemPrice[0]->price, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    } else {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItemPrice[6]->price.'">'. number_format($serviceItemPrice[6]->price, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    }
                } else if ($data['usage_type'] == 2) {
                    if ($serviceItem->service_item_type == 1) {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItemPrice[5]->price.'">'. number_format($serviceItemPrice[5]->price, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    } else if ($serviceItem->service_item_type == 2) {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItemPrice[4]->price.'">'. number_format($serviceItemPrice[4]->price, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    } else {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItemPrice[7]->price.'">'. number_format($serviceItemPrice[7]->price, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    }
                } else if ($data['usage_type'] == 3) {
                    if ($data['country_id'] == 19) {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItemPrice[2]->price.'">'. number_format($serviceItemPrice[2]->price, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    } else {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItemPrice[3]->price.'">'. number_format($serviceItemPrice[3]->price, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    }
                } else {
                    $serviceItemList[] = '<span class="text-right" style="display: block;margin-bottom: 12px;"> Select User Type</span>';
                }
                ...
                if ($data['country_id'] == 19) {
                    $serviceItemList[] = '<div class="remove_select'.$serviceInventoryItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceInventoryItem->service_id.'"><input type="hidden" class="total" value="'.$serviceInventoryItem->price.'">'. number_format($serviceInventoryItem->price, 2).' BDT <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceInventoryItem->id.')">X</a></div>';
                } else {
                    $serviceItemList[] = '<div class="remove_select'.$serviceInventoryItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceInventoryItem->service_id.'"><input type="hidden" class="total" value="'.$serviceInventoryItem->price_dollor.'">'. number_format($serviceInventoryItem->price_dollor, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceInventoryItem->id.')">X</a></div>';
                }
            } else {
                if ($data['usage_type'] == 1) {
                    if ($data['country_id'] == 19) {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItem->price_bdt_org.'">'. number_format($serviceItem->price_bdt_org, 2).' BDT <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    } else {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItem->price_usd_org.'">'. number_format($serviceItem->price_usd_org, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    }

                    // $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItem->price_usd_org.'">'. number_format($serviceItem->price_usd_org, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                } else if ($data['usage_type'] == 2) {
                    if ($data['country_id'] == 19) {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItem->price_bdt_personal.'">'. number_format($serviceItem->price_bdt_personal, 2).' BDT <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    } else {
                        $serviceItemList[] = '<div class="remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'"><input type="hidden" class="total" value="'.$serviceItem->price_usd_personal.'">'. number_format($serviceItem->price_usd_personal, 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                    }
    ...
    public function getInventoryItemsByServiceValue(Request $request)
    {
        ...
        foreach ($finalItems as $key => $item) {
            ...
            $serviceItemList[] = '<div class=" mb-3 remove_select'.$serviceItem->id.' selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_item_id.'">'. $serviceItem->serviceItem->item_name_en.'</div>';
            
            $serviceItemList[] = '<div class=" mb-3 remove_select'.$serviceItem->id.' selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_item_id.'">'. $serviceItem->title.'<input type="hidden" class="selected_serviceItem" value="'.$serviceItem->id.'"/><input type="hidden" name="service_item_id[]" value="'.$serviceItem->service_item_id.'"/></div>';

            if ($data['country_id'] == 19) {
                
                    $serviceItemList[] = '<div class=" mb-3 remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'" data-price_dollor="'.$serviceItem->price_dollor.'" data-price="'.$serviceItem->price.'"><input type="hidden" class="total" value="'.($serviceItem->price * ($request->quantity[$key] ?? 1)).'">'. number_format(($serviceItem->price * ($request->quantity[$key] ?? 1)), 2).' BDT <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                
            } else {
                
                    $serviceItemList[] = '<div class=" mb-3 remove_select'.$serviceItem->id.' text-right selectitems'.$data["service_id"].' selectservice'.$serviceItem->service_id.'" data-price_dollor="'.$serviceItem->price_dollor.'" data-price="'.$serviceItem->price.'"><input type="hidden" class="total" value="'.($serviceItem->price_dollor * ($request->quantity[$key] ?? 1)).'">'. number_format(($serviceItem->price_dollor * ($request->quantity[$key] ?? 1)), 2).' USD <a class="btn-sm btn-danger text-white" style="cursor: pointer; padding: 0 0.5rem;" onclick="remove_select('.$serviceItem->id.')">X</a></div>';
                
            }

            $serviceItemList[] = '<input type="number" name="quantity[]" id="quantity'.$serviceItem->id.'" value="'.($request->quantity[$key] ?? 1).'" class="remove_select'.$serviceItem->id.' form-control mb-1" onkeyup="quantityPrice('.$serviceItem->id.', this)" style="padding: 2px 0.5rem; height: 30px;">';
        }
