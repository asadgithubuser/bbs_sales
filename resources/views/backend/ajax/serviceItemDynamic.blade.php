<script>
    ...
    $(".service_id").on('change', function(e){
        ...
        if(usage_type_val == null){
            Swal.fire('Please, choose an user type!', '', 'warning');
            $('.service_id').prop("checked", false);
    ...
    $('.inventory_item_list').on('click', function() {
    ...
    function inventoryItemSelect(id){
        ...
        let usage_type_val = $("input[name='usage_type']:checked").val();
        let country_id = $("#country_id").val();
        let service_item_id_value = $('.service_item_select').val();
        let quantity = arr;
// console.log(quantity);
        $.ajax({
            ...
            success:function(response){
                ...
                $('.select_service_item_list').html('');
                $('.select_service_price_list').html('');
                $('.select_service_quantity_list').html('');
                ...
                $.each(response, function(){
                    ...
                    $('.select_service_item_list').append(response[i+1]);
                    $('.select_service_price_list').append(response[i+2]);
                    $('.select_service_quantity_list').append(response[i+3]);
                    i=i+4;
                });
    ...
    $('#country_id').on('change', function(){
        ...
        if (country_id == 19) {
            $('#inv_currency_label').text('BDT');
        } else {
            $('#inv_currency_label').text('USD');
        }
    ...
    function quantityPrice(id, that) {
        // let quantity = $(that).closest('tr').find('#quantity'+id).val();
        // // let price = $(that).closest('tr').find('.selectitems'+id+' .total').val();
        // let price_dollor = $(that).closest('tr').find('.selectitems'+id).attr('data-price_dollor');
        // let price_bd = $(that).closest('tr').find('.selectitems'+id).attr('data-price');

        // let country_id = $('#country_id').find(":selected").val();
