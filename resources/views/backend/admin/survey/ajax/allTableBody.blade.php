    ...
    <tbody>
        @if ($lists->count() > 0)
            ...
            @foreach ($lists as $list)
                ...
                <td class="text-left">{{ $list->surveyCountDivi($list->division_id)}}</td>
                ...
                <td>
                    ...
                    <a href="{{ route('admin.processingList.showAllDistrict',['list'=>$list->division_id,'form'=>$list->survey_form_id]) }}" class="btn btn-warning btn-sm">Details</a>
                    <a href="{{route('admin.processingList.farmerReportOk', ['list' =>$list, 'division_id' => $list->division_id] )}}" class="btn btn-info btn-sm">Report</a>
