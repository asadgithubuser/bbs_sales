        ...
        @if ($lists->count() > 0)
            ...
            @foreach ($lists as $list)
                ...
                <td>
                    ...
                    <a href="{{ route('admin.processingList.showDistrict',['list'=>$list->district_id,'form'=>$list->survey_form_id]) }}" class="btn btn-warning btn-sm">Details</a>
                    <a href="{{route('admin.processingList.farmerReportDivision', ['list' =>$list, 'district_id' => $list->district_id] )}}" class="btn btn-info btn-sm">Report</a>
            ...
            @endforeach
            <a href="{{route('admin.processingList.farmerReportOk', ['list' =>$list, 'division_id' => $list->division_id] )}}" class="w3-btn w3-teal w3-shadow-black" style="box-shadow: 0 8px 16px 0 rgb(0 0 0 / 20%), 0 6px 20px 0 rgb(0 0 0 / 19%);">বিভাগের সকল তথ্য</a>
