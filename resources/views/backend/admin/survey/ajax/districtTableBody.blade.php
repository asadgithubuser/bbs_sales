        ...
        @if ($lists->count() > 0)
            ...
            @foreach ($lists as $list)
                ...
                <td>
                    ...
                    <a href="{{ route('admin.processingList.showUpazila',$list->upazila_id ) }}" class="btn btn-warning btn-sm">Details</a>
                    <a href="{{route('admin.processingList.farmerReportDistrict',['list' =>$list, 'district_id' => $list->district_id] )}}" class="btn btn-info btn-sm">Report</a>
            ...
            @endforeach
            <a href="{{route('admin.processingList.farmerReportDivision', ['list' =>$list, 'district_id' => $list->district_id] )}}" class="w3-btn w3-teal w3-shadow-black" style="box-shadow: 0 8px 16px 0 rgb(0 0 0 / 20%), 0 6px 20px 0 rgb(0 0 0 / 19%);">জেলার সকল তথ্য</a>
...
@push('stackScript')
    <script> 
        $(".forward_btn").click(function(e) {
            ...
            var url = '<a href="{{route("admin.processingList.forwardToApprove", ":id")}}" class="swal2-confirm swal2-styled" title="Forward">Confirm</a>';
