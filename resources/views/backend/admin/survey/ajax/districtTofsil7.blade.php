<table class="table table-separate table-head-custom table-checkable table-striped">
    ...
    <tbody>
        @if ($lists->count() > 0)
            ...
            @endphp
            <a href="{{ route('admin.processingList.showDivisionShankalan7',$list) }}" class="w3-btn w3-teal w3-shadow-black" style="box-shadow: 0 8px 16px 0 rgb(0 0 0 / 20%), 0 6px 20px 0 rgb(0 0 0 / 19%);">জেলার সকল তথ্য </a>
            ...
            @foreach ($lists as $list)
                <tr>
                    ...
                    <td>
                        ...
                        <a href="{{ route('admin.processingList.showDistrictShankalan7',$list) }}" class="btn btn-info btn-sm">Report</a>
...
@push('stackScript')
    <script> 
        ...
        $(".backward_btn").click(function(e) {
            var data_id = $(this).attr("data-id");
            var url = '<a href="{{route("admin.processingList.backwardToUpazila", ":id")}}" class="swal2-confirm swal2-styled" title="Backward">Confirm</a>';
            url = url.replace(':id', data_id );
