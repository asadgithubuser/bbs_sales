    ...
    <tbody>
        @if ($lists->count() > 0)
            ...
            @foreach ($lists as $list)
                <tr>
                    <td>{{ $i }}</td>
                    <td class="text-left">{{ $list->upazila ? $list->upazila->name_bd : '' }}</td>
                    ...
                    <td>
                        ...
                        <a href="{{ route('admin.processingList.showUpazilaTofsil9List', $list) }}" class="btn btn-info btn-sm">Details</a>
                    </td>
                @php
                    $i++;
                @endphp
