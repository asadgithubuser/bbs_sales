    ...
    <tbody>
        @if ($lists->count() > 0)
            ...
            @foreach ($lists as $list)
                ...
                <td class="text-left">{{ $list->surveyCountWage($list->upazila_id)}}</td>
                ...
                <td>
                    ...
                    {{-- <a href="{{ route('admin.processingList.upazilaCropCuttingDetails', $list) }}" class="btn btn-info btn-sm">Details</a> --}}
                    <a href="{{ route('admin.processingList.backwardToUpazila',$list) }}" class="btn btn-danger btn-sm">Backward</a>
                    {{-- <a href="{{ route('admin.processingList.showDistrictShonkolon6', $list) }}" class="btn btn-info btn-sm">Report</a> --}}
                    <a href="{{ route('admin.processingList.upazilaMonthlyWageDetails', $list) }}" class="btn btn-info btn-sm">Details</a>
                ...
                <div class="modal fade" id="exampleModalCenter{{$list->id}}" tabindex="-1" aria-labelledby="exampleModalCenterTitle" style="display: none;" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered modal-dialog-bottom" role="document">
                       <div class="modal-content">
                          ...
                          <form action="{{ route('admin.processingList.commentDistrictTofsil6', $list) }}" method="post">
                            @csrf
                ...
                {{-- end Modal --}}
            @endforeach
            <a href="{{route('admin.processingList.districtReportShankalan8',['list' =>$list, 'district_id' => $list->district_id] )}}" class="w3-btn w3-teal w3-shadow-black" style="box-shadow: 0 8px 16px 0 rgb(0 0 0 / 20%), 0 6px 20px 0 rgb(0 0 0 / 19%);">জেলার সকল তথ্য </a>
