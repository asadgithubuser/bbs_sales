<table class="table table-separate table-head-custom table-checkable table-striped">
    <thead>
        <tr>
            <th>#</th>
            <th class="text-left">Collector Name</th>
    ...
    <tbody>
        @if ($tofsil9datas->count() > 0)
            @php
                $i = ($tofsil9datas->currentPage() - 1) * $tofsil9datas->perPage() + 1;
            @endphp

            @foreach ($tofsil9datas as $list)
                <tr>
                    <td>{{ $i }}</td>
                    <td class="text-left">{{ $list->user ? ($list->user->first_name.' '.$list->user->middle_name.' '.$list->user->last_name) : '' }}</td>
                    
                    <td>{{ $listData->year ? $listData->year : ''}}</td>
                    <td>
                        
                        <a href="{{ route('admin.processingList.showUpazilaTofsil9', $list) }}" class="btn btn-info btn-sm">Report</a>
                        @if ($list->status == 2)
                            ...
                            <button class="btn btn-danger btn-sm w3-circle backward_btn" title="Backward" data-id="{{ $list->survey_process_list_id }}">Backward</button>
                        @endif
                    </td>
                @php
                    $i++;
