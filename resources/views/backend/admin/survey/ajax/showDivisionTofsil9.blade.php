    ...
    <tbody>
        @if ($lists->count() > 0)
            ...
            @foreach ($lists as $list)
                <tr>
                    <td>{{ $i }}</td>
                <td class="text-left">{{ $list->upazilaInfo ? $list->upazilaInfo->name_en : '' }}</td>
                <td class="text-left">{{ $list->surveyUpazilaData( $list->upazila_id)}}</td>
                ...
                <td>
                    ...
                    <a href="{{ route('admin.processingList.showUpazilaTofsil9List', $list->survey_process_list_id) }}" class="btn btn-info btn-sm">Details</a>
