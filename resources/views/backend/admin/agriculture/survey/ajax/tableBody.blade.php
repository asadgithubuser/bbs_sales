<table class="table table-separate table-head-custom table-checkable table-striped">
    ...
    <tbody>
        ...
        @php
            $i = (($surveyForms->currentPage() - 1) * $surveyForms->perPage() + 1);
        @endphp
            @foreach ($surveyForms as $survey)
            <tr>
                <td>{{$i}}</td>
                <td align="left">{{ucfirst($survey->template_name)}}</td>
                <td align="left">{{ucfirst($survey->display_name)}}</td>
                <td align="left">{{$survey->table_name}}</td>
                <td>
                ...
                <td>
                    ...
                    @can('edit_survey')
                    <a href="{{route('admin.survey.edit', $survey->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit">
...
@push('stackScript')
    <script> 
        $(".delete").click(function(e) {
            ...
            var url =  '<a href="{{route("admin.survey.delete",":id")}}" class="swal2-confirm swal2-styled" title="Delete">Confirm</a>';
