					...
					<div id="kt_header" class="header header-fixed">
						...
						<div class="container-fluid d-flex align-items-stretch justify-content-between">
							...
							<div class="topbar">
								...
								<div class="dropdown">
									@php
										// get this users unread notification count (Data sell service notifications)
										$current_user_id = Auth::user()->id;
										$unread = $newNotifications->where('notifiable_id', $current_user_id)->where('read_at', NULL)->latest()->get();
										$count = count($unread);

										// get this users unread notification count (Agriculture module notifications)
										$user = Auth::user();
										...
										$counts = $generateSurveyNotifications->where('receiver_id', $user->id)
												->where('receiver_designation_id', $user->designation_id)
												->where('status', 1)
												->where('read_status', 0)
												->get();
									...
									<div class="dropdown-menu p-0 m-0 dropdown-menu-right dropdown-menu-anim-up dropdown-menu-lg">
										<form>
											...
											<div class="d-flex flex-column pt-12 bgi-size-cover bgi-no-repeat rounded-top" style="background-image: url({{ asset('assets/media/misc/bg-1.jpg') }})">
												...
												<!--end::Title-->
												<!--begin::Tabs-->
												<ul class="nav nav-bold nav-tabs nav-tabs-line nav-tabs-line-3x nav-tabs-line-transparent-white nav-tabs-line-active-border-success mt-3" role="tablist" style="width: 100%;">
													<li class="nav-item" style="text-align: center; width: {{ Auth::user()->role_id == 10 ? '100%': '50%'}} ; position: relative;">
														<a class="nav-link active show" data-toggle="tab" href="#topbar_notifications_data" style="display: block;">Data Sale</a>
														@if ($count > 0)
															<span style="color:white; font-weight: bold; margin-bottom: 25px; background: red; height: 23px; width: 20px; text-align: center; border-radius: 10px;position: absolute; right: 25%; top: 5%;">{{$count}}</span>
														@else
															<span style="color:white; font-weight: bold; margin-bottom: 25px; background: green; height: 23px; width: 20px; text-align: center; border-radius: 10px;position: absolute; right: 25%; top: 5%;">0</span>
														@endif
													</li>
													@if (Auth::user()->role_id != 10)
														
													<li class="nav-item" style="text-align: center; width: 50%; position: relative;">
														<a class="nav-link" data-toggle="tab" href="#topbar_notifications_survey" style="display: block;">Agriculture Survey</a>
														@if ($counter > 0)
															<span style="color:white; font-weight: bold; margin-bottom: 25px; background: red; height: 23px; width: 20px; text-align: center; border-radius: 10px;position: absolute; right: 0; top: 5%;">{{$counter}}</span>
														@else
															<span style="color:white; font-weight: bold; margin-bottom: 25px; background: green; height: 23px; width: 20px; text-align: center; border-radius: 10px;position: absolute; right: 0; top: 5%;">0</span>
														@endif
													</li>
													@endif
												</ul>
											...
											<div class="tab-content">
												<!--begin::Tabpane-->
												<div class="tab-pane active show" id="topbar_notifications_data" role="tabpanel">
													...
													<div class="navi navi-hover scroll" data-scroll="true" data-height="330" data-mobile-height="200">
														...
														@php
															...
															$oldNotifis = $oldNotifications->where('notifiable_id', $current_user_id)->where('read_at', '!=', NULL)->latest()->get();
														@endphp
												...
												@if (Auth::user()->role_id != 10)
												<!--begin::Tabpane-->
												<div class="tab-pane" id="topbar_notifications_survey" role="tabpanel">
													<!--begin::Nav-->
													<div class="navi navi-hover scroll" data-scroll="true" data-height="330" data-mobile-height="200">
														...
														@if (count($actives) > 0)
                											@foreach($actives as $active)
																...
																@if ($active->read_status == 0)
																	<!--begin::Item-->
																	<a href="{{route('gotoAgriNotification', $active->id)}}" class="navi-item" style="background: #EBF9CF">
																		<div class="navi-link">
																			<div class="navi-text">
																	...
																	<!--end::Item-->	
																@else
																	@if ($user->role_id == 9 AND $user->designation_id == 9)
																		{{-- <div class="d-flex flex-center text-center text-muted min-h-200px">All caught up!
																			<br />No new notifications.
																		</div> --}}
																	@else
																		<!--begin::Item-->
																		<a href="{{route('gotoAgriNotification', $active->id)}}" class="navi-item">
																			<div class="navi-link">
																				<div class="navi-text">
																					<div class="font-weight-bold">{{$active->data}}</div>
																					<div class="text-muted">{{ date('d M, Y h:i a', strtotime($active->created_at)) }}</div>
								...
								<div class="topbar-item">
									<div class="btn btn-icon btn-icon-mobile w-auto btn-clean d-flex align-items-center btn-lg px-2" id="kt_quick_user_toggle">
										...
										<span class="symbol symbol-lg-35 symbol-25 symbol-light-success">
											<span class="symbol-label font-size-h5 font-weight-bold">{{ ucfirst(substr(Auth::user()->first_name, 0, 1)) }}</span>
										</span>
									</div>
								</div>
								<!--end::User-->
