    ...
    <!--begin::Header-->
        <div class="d-flex align-items-center mt-5">
            <div class="symbol symbol-100 mr-5">
                @if (Auth::user()->photo)
                    <div class="symbol-label" style="background-image:url('{{ asset('storage/users/'.Auth::user()->photo) }}')"></div>
            ...
            <div class="d-flex flex-column">
                ...
                <div class="text-muted mt-1">{{ Auth::user()->designation ? Auth::user()->designation->name_en : '' }}</div>
                <div class="text-muted mt-1">{{ Auth::user()->office ? Auth::user()->office->title_en : '' }}</div>
                ...
                @can('edit_profile')
                    ...
                    @endcan
                <div class="navi mt-2">
                    <a href="{{ route('admin.user.editPassword', ['username'=>Auth::user()->username,'user'=>Auth::user()]) }}" class="btn btn-block btn-sm btn-light-warning font-weight-bolder py-2 px-5">Change Password</a>
                </div>
